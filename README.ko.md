...
**보안 하드닝 + 서브에이전트 UX + 안정성**
- **프로젝트 훅 게이트 (보안)** — 열린 저장소의 `.claude/settings.json` / `.pawn/hooks.json` 훅은 기본으로 실행되지 않습니다. 이 파일은 신뢰할 수 없는 저장소와 함께 들어오며 임의 셸 명령을 실행할 수 있습니다. Settings → Hooks에서 직접 켤 수 있고, 사용자 스코프 훅(~/.claude, Pawn 디렉터리)은 항상 동작합니다. `npm audit` 클린 (알려진 취약점 0건)
- **서브에이전트 라이브 상태 인라인** — 채팅에 접을 수 있는 바(“에이전트 N명 작업 중…”)로 표시되며 펼치면 각 런을 확인할 수 있습니다. 우측 패널 Agents 탭은 이력용으로 유지되고, 작업 시작 시 패널을 강제로 열지 않습니다
- **서브에이전트 브라우저 패널** — 서브에이전트 브라우징이 모두 끝나면 사이드 패널을 닫습니다(서브에이전트가 연 경우만 — 사용자가 열었거나 보고 있는 패널은 건드리지 않음). 순차 파이프라인 플리커 방지를 위해 디바운스 적용
- **안정성** — 서브에이전트 런은 예상치 못한 크래시에도 항상 종료 상태가 됩니다(유령 “running” 항목 없음). DB를 못 쓰는 환경에서는 사이드바 세션 검색이 인메모리 목록으로 폴백
- **UI** — 계획/에이전트/리뷰 위젯이 채팅 컬럼 폭에 정렬되고, 한국어 UI는 “도우미” 대신 “에이전트”를 사용합니다

### v0.11.0 — 큐/스티어 단순화 + 세션 검색 + 오버레이 하드닝
- **큐/스티어 단순화** — 컴포저 토글 제거. 전송은 설정에서 지정한 모드를 따르며, 큐 모드 실행 중에는 중지 버튼 옆에 작은 **스티어** 버튼이 나타나 입력 중인 내용을 즉시 전송합니다
- **세션 검색** — 사이드바가 모든 세션의 제목과 **메시지 내용**을 DB에서 검색하므로, 한 번도 열지 않은 세션도 검색 결과에 나타납니다
- **오버레이 하드닝** — 환경설정을 열면 임베디드 브라우저(렌더러 z-index로 가릴 수 없는 네이티브 뷰)를 숨기고 닫으면 복원합니다. 브라우저 페이지와 탭은 유지됩니다

---

### v0.10.0 — 멀티 탭 브라우저 + 서브에이전트 병렬 브라우징 + `research_report`
