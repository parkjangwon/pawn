...
export function buildIssuePrPlaybook(opts: {
  ...
  return [
    ...
    `2. Local prep: git_status, git_log(limit=5), branch name via shell_exec only if needed.`,
    `3. Locate code: repo_map or codebase_search + grep_search; read_file before edit.`,
    `4. Implement the smallest fix; prefer edit_file; update_plan for multi-step.`,
    `5. Verify: run_checks(kind="typecheck") then kind="test" if available.`,
    `6. Review: git_diff, git_pr_ready.`,
    `7. Open PR only if user asked or clearly implied: github_create_pull / gitlab_create_merge_request.`,
